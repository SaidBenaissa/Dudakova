...
class WPML_Simple_Language_Selector extends WPML_SP_User {
	...
	function render( $options = array() ) {
		...
		?>
			<select
				<?php if( $options[ 'id' ] ) { echo ' id="' . $options[ 'id' ] . '"'; } ?>
				<?php if( $options[ 'name' ] ) { echo ' name="' . $options[ 'name' ] . '"'; } ?>
				class="js-simple-lang-selector <?php echo $options[ 'class' ]; ?>"
				<?php echo $data; ?>
				<?php echo $disabled; ?>
				style="<?php echo $options[ 'style' ]; ?>"
				>
				<?php if( $options[ 'show_please_select' ] ): ?>
					<option value="" <?php if( $options[ 'selected' ] == '' ) echo 'selected="selected"'; ?>><?php echo $options[ 'please_select_text' ]; ?></option>
				<?php endif; ?>
				<?php foreach( $languages as $lang ): ?>
					<option value="<?php echo $lang[ 'code' ]; ?>" <?php if( $options[ 'selected' ] == $lang[ 'code' ] ) echo 'selected="selected"'; ?>
					        data-flag_url="<?php echo $this->sitepress->get_flag_url( $lang[ 'code' ] ); ?>"
							data-status="<?php echo in_array( $lang[ 'code' ], array_keys( $active_languages ) ) ? 'active' : ''; ?>"
							>
