...
class WPML_Meta_Boxes_Post_Edit_HTML extends WPML_SP_And_PT_User {
	...
	private function post_edit_languages_duplicate_of() {
		...
		if ( $duplicate_original_id ) {
			...
			<div class="icl_cyan_box"><?php
				printf( __( 'This document is a duplicate of %s and it is maintained by WPML.', 'sitepress' ), '<a href="' . get_edit_post_link( $duplicate_original_id ) . '">' . get_the_title( $duplicate_original_id ) . '</a>' );
				?>
				...
				<i><?php printf( __( 'WPML will no longer synchronize this %s with the original content.', 'sitepress' ), $this->post->post_type ); ?></i>
			</div>
		<?php
		}
	}
	...
	private function post_edit_languages_dropdown() {
		...
		<div id="icl_document_language_dropdown" class="icl_box_paragraph">
			<p>
				<label for="icl_post_language"><strong><?php printf( __( 'Language of this %s', 'sitepress' ),
				                                                     $this->post_type_label ); ?></strong></label>
			</p>
			...
			<select name="icl_post_language" id="icl_post_language" <?php echo $disabled_language; ?>>
				...
				foreach ( $active_langs as $code => $lang ) {
					...
					$selected_language = selected( true,
					                               $this->is_selected_lang( $code, $this->selected_language ),
					                               false );
					?>
					<option value="<?php echo $code ?>" <?php echo $selected_language; ?>>
			...
			<input type="hidden" name="icl_trid" value="<?php echo $this->get_trid() ?>"/>
		</div>
		<?php
	}
		...
		if($this->post->post_status != 'auto-draft') {
			...
			if ( count( $this->get_translations() ) === 1 && count( $this->sitepress->get_orphan_translations( $trid, $this->post->post_type, $current_language ) ) > 0 ) {
				...
				<div id="icl_document_connect_translations_dropdown" class="icl_box_paragraph">
					...
					<input type="hidden" id="icl_connect_translations_post_id" name="icl_connect_translations_post_id" value="<?php echo $this->post->ID; ?>"/>
					<input type="hidden" id="icl_connect_translations_trid" name="icl_connect_translations_trid" value="<?php echo $trid; ?>"/>
					<input type="hidden" id="icl_connect_translations_post_type" name="icl_connect_translations_post_type" value="<?php echo $this->post->post_type; ?>"/>
					<input type="hidden"
					       id="icl_connect_translations_language"
					       name="icl_connect_translations_language"
					       value="<?php echo $current_language; ?>"/> <?php wp_nonce_field( 'get_orphan_posts_nonce', '_icl_nonce_get_orphan_posts' ); ?>
				</div>

				<div class="hidden">
					<div id="connect_translations_dialog"
					     title="<?php _e( 'Choose a post to assign', 'sitepress' ); ?>"
						...
						<div class="posts-found js-posts-found">
							<label id="post-label" for="post_search">
								<?php _e( 'Type a post title', 'sitepress' ); ?>: </label>
					...
					<div id="connect_translations_dialog_confirm"
					     title="<?php echo esc_attr( __( 'Connect this post?', 'sitepress' ) ); ?>"
					     data-cancel-label="<?php echo esc_attr( __( 'Cancel', 'sitepress' ) ); ?>"
					     data-assign-label="<?php echo esc_attr( __( 'Assign', 'sitepress' ) ); ?>">
						<p>
							<span class="ui-icon ui-icon-alert"></span> <?php _e( 'You are about to connect the current post with these following posts', 'sitepress' ); ?>: </p>
		...
		<div id="translation_of_wrap">
			...
			if ( $this->is_a_translation() ) {
				...
				<div id="icl_translation_of_panel" class="icl_box_paragraph">
					<label for="icl_translation_of"><?php _e( 'This is a translation of', 'sitepress' ); ?></label>&nbsp;
	...
	private function translation_summary( $status_display ) {
		...
		<div class="icl_box_paragraph">
			...
			(<a class="icl_toggle_show_translations" href="#"
			    <?php if ( $not_show_flags ): ?>style="display:none;"<?php endif; ?>><?php _e( 'hide',
			                                                                                   'sitepress' ) ?></a><a
				class="icl_toggle_show_translations"
				href="#"
				<?php if ( ! $not_show_flags ): ?>style="display:none;"<?php endif; ?>><?php _e( 'show', 'sitepress' ) ?></a>)
			...
			<table width="100%" class="icl_translations_table" id="icl_translations_table"
			       <?php if ( $not_show_flags ): ?>style="display:none;"<?php endif; ?>>
				<?php
				$odd_even     = 1;
				$active_langs = $this->sitepress->get_active_languages();
				$translations = $this->get_translations();
				?>
				<?php foreach ( $active_langs as $lang ): if ( $this->selected_language == $lang['code'] ) {
					continue;
				} ?>
	...
	private function init_translation_of_options() {
		...
		if ( $this->handle_as_original() && apply_filters( 'wpml_language_is_active',
		                                                   null,
		...
		<tr <?php echo $row_class; ?>>
			...
			if ( ! isset( $translations[ $lang[ 'code' ] ] ) ) {
				...
				<td align="right">
					...
					?>
					<input<?php disabled( true, $disabled_duplication ); ?> type="checkbox" name="icl_dupes[]" value="<?php echo $lang[ 'code' ] ?>" title="<?php echo $disabled_duplication_title ?>"/>
				</td>
	...
	private function copy_from_original( $post ) {
		$trid        = $this->get_trid();
		$source_lang = filter_var( isset( $_GET['source_lang'] ) ? $_GET['source_lang'] : '',
		                           FILTER_SANITIZE_FULL_SPECIAL_CHARS );
		$source_lang = $source_lang === 'all' ? $this->sitepress->get_default_language() : $source_lang;
		$lang        = filter_var( isset( $_GET['lang'] ) ? $_GET['lang'] : '',
		                           FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	...
	private function display_copy_from_button( $source_lang, $source_lang_name, $post, $trid ) {
		...
		echo '<input id="icl_cfo" class="button-secondary" type="button" value="' . sprintf(
				...
				onclick="icl_copy_from_original(\'' . $source_lang . '\', \'' . $trid . '\')"'
	...
	private function get_element_title( $source_element_id ) {
		...
		if ( $source_element_id && $source_element_id != $this->post->ID ) {
			...
			if ( trim( $element_title ) === '' ) {
				$element_title = '{' . __( 'Post without a title', 'sitepress' ) . '}';
			}
		}
