...
class WPML_Slug_Translation {
			...
			if ( !isset( $this->translated_slugs[ $post_type ][ $language ] ) ) {
				...
				foreach( $this->sitepress->get_active_languages() as $lang ) {
					if ( ! isset( $this->translated_slugs[ $post_type ][ $lang[ 'code' ] ] ) ) {
						$this->translated_slugs[ $post_type ][ $lang[ 'code' ] ] = '';
					}
				}
				
			}
			$slug = $this->translated_slugs[ $post_type ][ $language ] ? $this->translated_slugs[ $post_type ][ $language ] : $slug;
		}
	...
	function filter_pre_get_posts( $query ) {
		...
		foreach ( $query->query as $slug => $post_name ) {
			if ( in_array( $slug, $all_slugs_translations ) ) {
				$new_slug = $this->get_translated_slug( $slug, $query->query[ 'post_type' ], 'en' );
	...
	static function activate_slug_translation_action( $post_type, $slug = null ) {
		...
		if( empty( $sitepress_settings[ 'posts_slug_translation' ][ 'on' ] ) || empty( $sitepress_settings[ 'posts_slug_translation' ][ 'types' ][ $post_type ] ) ) {
			$iclsettings[ 'posts_slug_translation' ][ 'on' ] = 1;
			$iclsettings[ 'posts_slug_translation' ][ 'types' ][ $post_type ] = 1;
			$sitepress->save_settings( $iclsettings );
		}
	}
