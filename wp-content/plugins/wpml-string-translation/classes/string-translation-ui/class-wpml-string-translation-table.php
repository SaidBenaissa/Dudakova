...
class WPML_String_Translation_Table {
	...
	public function render_string_row ( $string_id, $icl_string ) {
		...
		$lang_not_active = '';
		if ( ! in_array( $icl_string[ 'string_language' ], array_keys( $this->active_languages ) ) ) {
			$lang_not_active = ' js-lang-not-active';
		}

		?>
			<tr valign="top">
				<td>
					<input class="icl_st_row_cb<?php echo $lang_not_active; ?>"
						   type="checkbox"
						   value="<?php echo $string_id ?>"
						   data-language="<?php echo $icl_string[ 'string_language' ]; ?>" />
				</td>
				...
				<td width="70%">
					...
					<div class="icl-st-inline">
						<?php foreach( $this->active_languages as $lang ): if( $lang['code'] === $icl_string[ 'string_language' ] ) continue;  ?>
				...
				echo apply_filters( 'wpml_string_status_text',
									WPML_ST_String_Statuses::get_status( (int) $icl_string[ 'status' ] ),
									$string_id );
				?>
				</span>
