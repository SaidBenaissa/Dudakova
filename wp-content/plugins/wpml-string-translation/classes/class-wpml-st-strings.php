...
class WPML_ST_Strings {
	...
	public function get_string_translations() {
		...
		if ( $current_user_can_translate_strings ) {
			...
			$sql_prepared .= " AND TRIM(s.value) != '' ORDER BY s.id DESC LIMIT {$offset},{$limit}";
			...
			$res = $this->wpdb->get_results($sql_query . " AND TRIM(s.value) != '' ORDER BY string_id DESC LIMIT {$offset},{$limit}", ARRAY_A);
