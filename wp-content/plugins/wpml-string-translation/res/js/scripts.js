...
function icl_st_update_languages(){
    if(!jQuery('#icl_tm_languages input:checked').length){
        ...
        if(jQuery('.icl_st_row_cb:checked, .check-column input:checked').length &&
				jQuery('.js-lang-not-active:checked').length == 0) {
...
function icl_st_update_checked_elements(){    
    ...
    if(!jQuery('.icl_st_row_cb:checked, .check-column input:checked').length){
        jQuery('#icl_st_delete_selected, #icl_send_strings, #icl_st_change_lang_selected').attr('disabled','disabled');
		WPML_String_Translation.translation_basket.clear_message();
    }else{        
        jQuery('#icl_st_delete_selected, #icl_st_change_lang_selected').removeAttr('disabled');
